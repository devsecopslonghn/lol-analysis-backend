...
- Semantic movement status: `unknown`
...
`16.14.794.5912`. Its movement configuration is useful as a legacy reference
for identifying candidate fields, but it is not an exact decoder for client
`16.17.810.4348`. The replay report therefore records this profile as
`legacy_candidate_only` and never emits coordinates, routes, ganks or entity
ownership from it.
